...
#pragma omp parallel for
    ...
    void sdk_handle(rs2::frame &f, rs2::frame_source &src) {
        ...
        cv::Mat matDepth(cv::Size(depth_frame.get_width(), depth_frame.get_height()), CV_16U,
                         (void *) depth_frame.get_data(), cv::Mat::AUTO_STEP);
        cv::Mat matGray(cv::Size(ir_frame.get_width(), ir_frame.get_height()), CV_8U, (void *) ir_frame.get_data(),
                        cv::Mat::AUTO_STEP);
        ...
        auto res_depth = src.allocate_video_frame(_output_depth_profile, depth_frame, 0,
                                                  newW, newH, depth_frame.get_bytes_per_pixel() * newW,
