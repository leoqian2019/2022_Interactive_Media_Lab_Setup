...
public:
  ...
  Maker(const char *broadcastIp = "127.0.0.1")
      : broadcastIp(broadcastIp), done(false), waitingToStart(true),
        queue(new Queue<STATE>), shouldLog(false) {}
...
public:
  ...
  Taker()
      : done(false), waitingToStart(true), queue(new Queue<STATE>),
        shouldLog(false) {}
